...
export default function Gaokao({fontSize}) {
  ...
  let weekdays_list = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

  useEffect(() => {
    const fetchSource = async () => {
      let json = await fetch('/gaokao');
  ...
  return (
      <div className={'box'}>
        <p style={{fontSize: fontSize}}>{moment(Date.now()).
            format('YY/MM/DD')}&nbsp;{weekdays_list[weekday]}</p>
        <Spin spinning={loading}>
          {!loading && (
              <p style={{fontSize: fontSize}}>
                距离{countName}<span
                  style={{color: 'red', fontSize: fontSize}}>{gaokao}</span>天
              </p>
          )}
        </Spin>
